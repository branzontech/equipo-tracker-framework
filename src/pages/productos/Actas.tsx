...
const Actas = () => {
  ...
  const filtrarActas = () => {
    return actasData.filter((acta) => {
      ...
      const cumpleEstado = estadoFiltro === "todos" || acta.estado === estadoFiltro;
        ...
        <Card key={acta.id} className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div>
                <span className="font-semibold text-xs uppercase tracking-wider text-gray-500">Tipo:</span>{" "}
                <span className="capitalize">{getTipoLabel(acta.tipo)}</span>
              </div>
              <p className="text-sm text-gray-600 line-clamp-2">{acta.descripcion}</p>
          ...
          <div className="py-6 space-y-6">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Detalles generales</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{getTipoLabel(currentActa.tipo)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Fecha</p>
                  <p className="font-medium">{format(currentActa.fecha, "PPP")}</p>
                </div>
                <div>
                ...
                {currentActa.tipo === "prestamo" && currentActa.fechaDevolucion && (
                  <div>
                    <p className="text-sm text-gray-500">Fecha de devolución</p>
                    <p className="font-medium">{format(currentActa.fechaDevolucion, "PPP")}</p>
                  </div>
                )}
                
                {currentActa.tipo === "traslado" && currentActa.regionalDestino && (
                  <div>
                    <p className="text-sm text-gray-500">Destino</p>
                    <p className="font-medium">{currentActa.regionalDestino} - {currentActa.bodegaDestino}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {currentActa.estado !== "finalizada" && (
                  <Button 
                    ...
                    size="sm" 
                    className="flex gap-2"
                    onClick={() => handleStatusChange(currentActa, "finalizada")}
                  >
                ...
                {currentActa.estado !== "en_proceso" && (
                  <Button 
                    ...
                    size="sm" 
                    className="flex gap-2"
                    onClick={() => handleStatusChange(currentActa, "en_proceso")}
                  >
            ...
            {currentActa.tipo === "prestamo" && (
              <>
                <Separator />
                <div className="space-y-4">
                  <h3 className="font-semibold">Acciones específicas de préstamo</h3>
                  <div className="space-y-2">
                    {currentActa.estado === "vigente" && (
                      <Button 
                    ...
                    {(currentActa.estado === "vigente" || currentActa.estado === "pendiente_devolucion") && (
                      <Button 
                        variant="secondary"
                        className="w-full flex gap-2"
            ...
            {(currentActa.estado === "vigente" || currentActa.estado === "en_proceso") && (
              <>
                <Separator />
                <Button 
                  variant="destructive"
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        ...
        <Select value={estadoFiltro} onValueChange={setEstadoFiltro}>
          ...
          <SelectContent>
            ...
            <SelectItem value="pendiente_devolucion">Pendiente devolución</SelectItem>
          </SelectContent>
        </Select>
      </div>
